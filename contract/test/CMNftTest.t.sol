...
contract CMNftTest is StdCheats, Test {
    ...
    function testTokenIdIncrement() public {
        ...
        vm.prank(bob);
        basicNft.mint();
        assertEq(basicNft.ownerOf(2), bob);
        
        // 第三次mint
        vm.prank(alice);
        basicNft.mint();
        assertEq(basicNft.ownerOf(3), alice);
    }

    function testDemoMint() public {
        Demo demo = new Demo();
        
        // 测试第一次mint
        demo.mint();
        assertEq(demo.ownerOf(1), address(this));
        
        // 测试第二次mint
        demo.mint();
        assertEq(demo.ownerOf(2), address(this));
        
        // 验证下一个tokenId
        assertEq(demo.getCurrentTokenId(), 3);
    }
}
